...
class Users {
	...
	function __construct(){
		...
		$dbName = 'facebook'; //Define database name
	...
	function checkUser($oauth_provider,$oauth_uid,$fname,$lname,$gender,$locale,$picture){
		...
		if(mysqli_num_rows($prev_query)>0){
			$update = mysqli_query($this->connect,"UPDATE $this->table_name SET oauth_provider = '".$oauth_provider."', oauth_uid = '".$oauth_uid."', fname = '".$fname."', lname = '".$lname."', gender = '".$gender."', locale = '".$locale."', picture = '".$picture."', modified = '".date("Y-m-d H:i:s")."' WHERE oauth_provider = '".$oauth_provider."' AND oauth_uid = '".$oauth_uid."'");
		}else{
